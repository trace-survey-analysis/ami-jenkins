multibranchPipelineJob('db-backup-operator-build-and-publish') {
    ...
    branchSources {
        branchSource {
            source {
                github {
                    id('db-backup-operator-build-and-publish')
                    credentialsId('github-pat')
                    configuredByUrl(true)
                    repoOwner('cyse7125-sp25-team03')
                    repository('db-backup-operator')
                    repositoryUrl('https://github.com/cyse7125-sp25-team03/db-backup-operator.git')
                }
            }
        }
    }
